...
document.addEventListener('DOMContentLoaded', function () {
    faidiAuth.session().then(function (user) {
        if (user) {
            currentUser = user;
            showAdminPanel();
            initializeAdmin();
...
function handleLogin(event) {
    ...
    faidiAuth.login(user, pass).then(function (loggedIn) {
...
function killAllSessions() {
    ...
    faidiAuth.killAll().then(function () {
...
function loadUsers() {
    ...
    faidiAuth.listUsers().then(function (list) {
        usersList = list;
        renderUsersTable();
    }).catch(function () {
...
function saveUser(event) {
    ...
    faidiAuth.saveUser(payload).then(function () {
...
function removeUser(username) {
    ...
    faidiAuth.deleteUser(username).then(function () {
...
function resetUserPassword(username) {
    ...
    faidiAuth.saveUser({ username: username, name: u.name, role: u.role, password: String(password || '5555') }).then(function () {
